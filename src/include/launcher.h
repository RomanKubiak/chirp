...
private:
    ...
    static constexpr const char *kLauncherStatePath = "/userdata/launcher_state.ini";
    ...
    bool     launcherMenuVisible_ = true;
    ...
    bool     buttonLongPressHandled_ = false;
    ...
    void launcherShowDiagnosticScreen();
    void launcherShowConfigScreen();
    void renderDiagnosticScreen();
    void renderConfigScreen();
    void stopScript(size_t idx);
    void launcherSwitchFocusContext();
    void launcherShowMenu();
    void launcherShowScriptOutput();
    void launcherToggleScriptRuntime();
    void launcherRequestReboot(bool bootloader);
    bool launcherHandleNavigation(bool prev, bool next, bool select, bool longPress);

    unsigned countRunningScripts() const;
    bool isScriptRunning(size_t index) const;
    bool isScriptFocused(size_t index) const;
    bool focusedScriptIsRunning() const;
    size_t launcherMenuItemCount() const;
    bool isDiagnosticIndex(size_t index) const;
    bool isRebootIndex(size_t index) const;
    bool isConfigIndex(size_t index) const;
